...
const waitValid = game.validateDslProgram({ steps: [{ op: 'wait', seconds: 4.5 }, { op: 'loop' }] });
assert.equal(waitValid.ok, true, waitValid);
assert.equal(waitValid.program.steps[0].seconds, 4.5, 'wait keeps validated seconds');
...
const parserWaitById = parseAssistantRequest('Bot 1 wait 5 seconds', game, { enableTemplates: true, loadout: ['starter_automation'] });
assert.equal(parserWaitById.dslAssignments?.[0]?.program?.steps?.[0]?.op, 'wait', parserWaitById);
assert.equal(parserWaitById.dslAssignments?.[0]?.program?.steps?.[0]?.seconds, 5, parserWaitById);
...
const parserAnyEligible = parseAssistantRequest('someone bring me a log', game, { enableTemplates: true, loadout: ['starter_automation'] });
assert.equal(parserAnyEligible.dslAssignments?.[0]?.assignee?.strategy, 'any_eligible', parserAnyEligible);
assert.equal(parserAnyEligible.dslAssignments?.[0]?.program?.steps?.[0]?.op, 'pick_up', parserAnyEligible);
const validatedAnyEligible = validateDslAssignments([{ assignee: { strategy: 'any_eligible' }, program: { steps: [{ op: 'wait', seconds: 1 }, { op: 'loop' }] } }], game, { loadout: ['starter_automation'] });
assert.equal(validatedAnyEligible[0].assignee.strategy, 'any_eligible', validatedAnyEligible);
...
const parserWaitByName = parseAssistantRequest('Lumberjack wait 2 seconds', game, { enableTemplates: true, loadout: ['starter_automation'] });
assert.equal(parserWaitByName.dslAssignments?.[0]?.botId, 1, parserWaitByName);
assert.equal(parserWaitByName.dslAssignments?.[0]?.program?.steps?.[0]?.seconds, 2, parserWaitByName);
...
assigned = game.assignCustomDslProgram({ botId: 1, program: { steps: [{ op: 'wait', seconds: 0.5 }, { op: 'loop' }] } });
assert.equal(assigned.ok, true, assigned);
const waitStartPc = bot1.runtime.pc;
tick(game, 0.2);
assert.equal(bot1.runtime.pc, waitStartPc, 'wait step should not advance before the requested duration');
tick(game, 0.4);
assert.equal(bot1.runtime.pc, waitStartPc, 'single-step repeating wait loops back to the first step after the requested duration');
assigned = game.assignCustomDslProgram({ botId: 1, program: { repeat: false, steps: [{ op: 'wait', seconds: 0.2 }] } });
assert.equal(assigned.ok, true, assigned);
tick(game, 0.3);
assert.equal(bot1.program, 'idle', bot1);
...
assigned = game.assignCustomDslProgram({ assignee: { strategy: 'any_eligible' }, program: { steps: [{ op: 'wait', seconds: 1 }, { op: 'loop' }] } });
assert.equal(assigned.ok, true, assigned);
assert.equal(assigned.bot.id, 3, assigned);
const busyWorker = game.findBot(3);
assert.equal(busyWorker.program, 'taught_loop', busyWorker);
const bot4Idle = game.findBot(4);
const occupyBot4 = game.assignCustomDslProgram({ botId: 4, program: { steps: [{ op: 'wait', seconds: 1 }, { op: 'loop' }] } });
assert.equal(occupyBot4.ok, true, occupyBot4);
assert.equal(bot4Idle.program, 'taught_loop', bot4Idle);
const anyEligibleMissing = game.assignCustomDslProgram({ assignee: { strategy: 'any_eligible' }, program: { steps: [{ op: 'wait', seconds: 1 }, { op: 'loop' }] } });
assert.equal(anyEligibleMissing.ok, false, anyEligibleMissing);
assert.match(anyEligibleMissing.error, /No idle standard worker bot is available/);
