...
with socketserver.TCPServer(("127.0.0.1", 0), functools.partial(QuietHandler, directory=str(ROOT))) as server:
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_function("() => document.querySelector('.settings-tabs').hidden")
        assert page.evaluate("window.getGameState().paused") is True
        assert page.evaluate("() => !document.getElementById('settingsMainPanel').hidden")
        assert page.evaluate("() => document.querySelector('.settings-tabs').hidden")
        page.locator("#openSettingsTabBtn").click()
        page.wait_for_function("() => !document.querySelector('.settings-tabs').hidden")
        page.wait_for_function("() => document.querySelector('[data-settings-panel=\"controls\"]').classList.contains('is-active')")
        assert page.evaluate("() => document.getElementById('settingsMainPanel').hidden")
        page.locator("[data-settings-tab='save-load']").click()
        page.wait_for_function("() => document.querySelector('[data-settings-panel=\"save-load\"]').classList.contains('is-active')")
        assert page.locator("[data-save-flow-placeholder]").is_visible()
        assert page.locator("#saveGameBtn").is_hidden()
        assert page.locator("#loadGameBtn").is_hidden()
        page.locator("[data-save-flow-mode='save']").click()
        page.wait_for_function("() => !document.querySelector('[data-save-flow-panel=\"save\"]').hidden")
        assert page.locator("#saveGameBtn").is_visible()
        assert page.locator("#loadGameBtn").is_hidden()
        page.locator("[data-save-flow-mode='load']").click()
        page.wait_for_function("() => !document.querySelector('[data-save-flow-panel=\"load\"]').hidden")
        assert page.locator("#loadGameBtn").is_visible()
        assert page.locator("#saveGameBtn").is_hidden()
        page.keyboard.press("Escape")
        page.wait_for_function("() => document.querySelector('#settingsOverlay').hidden")
        page.keyboard.press("Escape")
        page.wait_for_function("() => !document.getElementById('settingsOverlay').hidden && !document.getElementById('settingsMainPanel').hidden")
