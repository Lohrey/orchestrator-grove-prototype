...
assert.ok(game.finishPlayerDeployLooseKit(game.player.target), 'loose kit deploys');
...
assert.ok(game.disassembleStructureToKit(sawbench, game.player), 'disassemble returns a kit');
...
assert.ok(game.finishPlayerDeployHeldKit({ itemType: 'sawbench_kit', x: 320, y: 320 }), 'held kit deploys');
