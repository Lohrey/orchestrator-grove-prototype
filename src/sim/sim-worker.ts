type SimPoint = { id: string | number; numericId?: number; type?: string; x?: number; y?: number };
type SimQuery = { type?: string; x?: number; y?: number };
type NumericQuery = Record<string, number>;
type WasmExports = {
  sim_core_version?: () => number;
  clear_points?: () => void;
  set_point?: (index: number, id: number, kind: number, x: number, y: number) => void;
  nearest_point?: (qx: number, qy: number, kind: number, len: number) => number;
  chunk_coord?: (value: number, chunkSize: number) => number;
  compute_next_step?: (x: number, y: number, tx: number, ty: number, dt: number, speed: number, close: number) => number;
  last_step_x?: () => number;
  last_step_y?: () => number;
};
...
const KIND: Record<string, number> = Object.freeze({ any: 0, log: 1, plank: 2, pole: 3, stick: 4, stone: 5, tree_seed: 6, crude_axe: 7, crude_pickaxe: 8, crude_shovel: 9, crude_hammer: 10, hemp: 11, hemp_seed: 12 });

function kindCode(type?: string): number {
  return type ? (KIND[type] || 0) : 0;
}

async function instantiateWasm(): Promise<WasmExports> {
...
function nearestItemWithJs(points: SimPoint[] = [], query: SimQuery = {}) {
...
async function nearestItem(points: SimPoint[] = [], query: SimQuery = {}) {
...
async function pathfind(query: NumericQuery = {}) {
  await loadWasm();
  return { waypoints: [{ x: Number(query.tx || 0), y: Number(query.ty || 0) }], backend: 'rust-wasm-direct-path' };
}

async function chunkForPoint(query: NumericQuery = {}) {
  const module = await loadWasm();
...
async function botTick(query: NumericQuery = {}) {
  const module = await loadWasm();
